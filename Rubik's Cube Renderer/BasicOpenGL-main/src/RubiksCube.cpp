...
void RubiksCube::rotateUpWall() {
    ...
    for (int i = 0; i < smallCubes.size(); ++i) {
        ...
        if (std::abs(cube->getPosition().z - (centerPosition.z + 1)) < epsilon  && 
            glm::length(cube->getPosition() - centerPosition) < 2) {
...
void RubiksCube::rotateDownWall() {
    ...
    for (int i = 0; i < smallCubes.size(); ++i) {
        ...
        // Check if the cube is part of the down wall (relative to the centerCube)
        if (std::abs(cube->getPosition().z - (centerPosition.z - 1)) < epsilon && glm::length(cube->getPosition() - centerPosition) < 1.9  ) { // Downward from centerCube (considering -1)
...
void RubiksCube::UpArrow() {
    for (SmallCube* cube : this->smallCubes) {
        ...
        glm::mat4 rotationMatrix = glm::rotate(glm::mat4(1.0f), glm::radians(this->Sensitivity), glm::vec3(1.0f, 0.0f, 0.0f));

        

        // Set the updated model matrix
        cube->setRotationMatrix(rotationMatrix * cube->getRotationMatrix());
    }
}

void RubiksCube::DownArrow() {
    for (SmallCube* cube : this->smallCubes) {
        // Create a rotation matrix for rotating around the negative X-axis
        glm::mat4 rotationMatrix = glm::rotate(glm::mat4(1.0f), glm::radians(-this->Sensitivity), glm::vec3(1.0f, 0.0f, 0.0f));
...
void RubiksCube::RightArrow() {
    for (SmallCube* cube : this->smallCubes) {
        ...
        glm::mat4 rotationMatrix = glm::rotate(glm::mat4(1.0f), glm::radians(this->Sensitivity), glm::vec3(0.0f, 1.0f, 0.0f));
...
void RubiksCube::LeftArrow() {
    for (SmallCube* cube : this->smallCubes) {
        // Create a rotation matrix for rotating around the negative Y-axis
        glm::mat4 rotationMatrix = glm::rotate(glm::mat4(1.0f), glm::radians(-this->Sensitivity), glm::vec3(0.0f, 1.0f, 0.0f));
